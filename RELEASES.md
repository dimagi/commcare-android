...
- **Add email address to PersonalID signup/recovery flow (WIP):**
  ...
  - **Email OTP screen:**
    ...
    - Type 5 digits — verify the Verify button stays disabled. Type the 6th digit — verification fires automatically. With a correct code:
        ...
        - **Existing user (legacy add email)**: a non-cancellable confirmation dialog appears with the title "Email Added" and message "Your email has been added successfully."; tapping OK closes the screen and the new email is visible on the user's HQ admin profile.
