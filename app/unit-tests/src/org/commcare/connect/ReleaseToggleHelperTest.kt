...
class ReleaseToggleHelperTest {
    ...
    // --- isEmailOtpVerificationActive(sessionData) -----------------------------------------

    @Test
    fun `isEmailOtpVerificationActive with session returns true when toggle is active in session`() {
        val sessionData =
            PersonalIdSessionData(
                featureReleaseToggles = listOf(buildToggle("email_otp_verification", active = true)),
            )

        assertTrue(ReleaseToggleHelper.isEmailOtpVerificationActive(sessionData))
    }

    @Test
    fun `isEmailOtpVerificationActive with session returns false when toggle is inactive in session`() {
        val sessionData =
            PersonalIdSessionData(
                featureReleaseToggles = listOf(buildToggle("email_otp_verification", active = false)),
            )

        assertFalse(ReleaseToggleHelper.isEmailOtpVerificationActive(sessionData))
    }

    @Test
    fun `isEmailOtpVerificationActive with session returns false when toggle is absent from session`() {
        val sessionData =
            PersonalIdSessionData(
                featureReleaseToggles = listOf(buildToggle("unrelated_feature", active = true)),
            )
    ...
    @Test
    fun `isEmailOtpVerificationActive with session returns false when session data is null`() {
        assertFalse(ReleaseToggleHelper.isEmailOtpVerificationActive(null as PersonalIdSessionData?))
    }

    // --- isEmailOtpVerificationActive(context) ---------------------------------------------

    @Test
    fun `isEmailOtpVerificationActive with context returns true when toggle is active in DB`() {
        every { ConnectAppDatabaseUtil.getReleaseToggles(context) } returns
            listOf(buildToggle("email_otp_verification", active = true))

        assertTrue(ReleaseToggleHelper.isEmailOtpVerificationActive(context))
    }

    @Test
    fun `isEmailOtpVerificationActive with context returns false when toggle is inactive in DB`() {
        every { ConnectAppDatabaseUtil.getReleaseToggles(context) } returns
            listOf(buildToggle("email_otp_verification", active = false))

        assertFalse(ReleaseToggleHelper.isEmailOtpVerificationActive(context))
    }

    @Test
    fun `isEmailOtpVerificationActive with context returns false when toggle is absent from DB`() {
        every { ConnectAppDatabaseUtil.getReleaseToggles(context) } returns
            listOf(buildToggle("unrelated_feature", active = true))

        assertFalse(ReleaseToggleHelper.isEmailOtpVerificationActive(context))
    }

    @Test
    fun `isEmailOtpVerificationActive with context returns false when DB returns empty list`() {
        every { ConnectAppDatabaseUtil.getReleaseToggles(context) } returns emptyList()
    ...
    @Test
    fun `isEmailOtpVerificationActive with context returns false when DB returns null`() {
        every { ConnectAppDatabaseUtil.getReleaseToggles(context) } returns null
