...
class ConnectLearningProgressFragmentTest {
    ...
    fun setUp() {
        ...
        every { anyConstructed<ConnectLearningProgressViewModel>().loadLearningProgress(any()) } returns Unit
        every { anyConstructed<ConnectLearningProgressViewModel>().learningProgress } returns
            androidx.lifecycle.MutableLiveData<DataState<ConnectJobRecord>>(DataState.Loading)
    }
    ...
    fun testFragment_isAddedToActivity() {
        ...
        val fragment = ConnectLearningProgressFragment.newInstance(false)
