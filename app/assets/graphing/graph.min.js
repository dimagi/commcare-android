...
if(type==="time"){formatXTooltip=d3.timeFormat(config.axis.x.tick.format);}
...
for(var yID in config.data.colors){if(type==="bar"){var bars=d3.selectAll(".c3-bars-"+yID+" path")["_groups"][0];for(var i=0;i<bars.length;i++){if(data.barColors[yID]&&data.barColors[yID][i]){bars[i].style.fill=data.barColors[yID][i];}
var opacity;if(data.barOpacities[yID]){opacity=data.barOpacities[yID][i];}
opacity=opacity||data.lineOpacities[yID];bars[i].style.opacity=opacity;}}else{var line=d3.selectAll(".c3-lines-"+yID+" path")["_groups"][0][0];if(line){line.style.opacity=data.lineOpacities[yID];}}
var legend=d3.selectAll(".c3-legend-item-"+yID+" path")["_groups"][0];if(!legend.length){legend=d3.selectAll(".c3-legend-item-"+yID+" line")["_groups"][0];}
...
var points=d3.selectAll(".c3-circles-"+yID+" path")["_groups"][0];if(!points.length){points=d3.selectAll(".c3-circles-"+yID+" circle")["_groups"][0];}
for(var i=0;i<points.length;i++){points[i].style.opacity=data.lineOpacities[yID];}}
for(var yID in data.areaColors){var area=d3.selectAll(".c3-areas-"+yID+" path")["_groups"][0][0];if(area){area.style.fill=data.areaColors[yID];area.style.opacity=data.areaOpacities[yID];}}
...
var circleSet=d3.selectAll(".c3-circles-"+yID);var circles=circleSet.selectAll("circle")["_groups"][0];if(!circles){return;}
for(var j=0;j<circles.length;j++){circles[j].style.opacity=0;appendSymbol(circleSet,circles[j].cx.baseVal.value,circles[j].cy.baseVal.value,symbol,circles[j].style.color);}}
function applyLegendShape(yID,symbol){if(symbol!=="none"){var legendItem=d3.selectAll(".c3-legend-item-"+yID);var line=legendItem.selectAll("line")["_groups"];if(!line||!line.length){return;}
line=line[0][0]
...
var d3Symbol=d3.symbolCircle;switch(symbol){case"cross":d3Symbol=d3.symbolCross;break;case"diamond":d3Symbol=d3.symbolDiamond;break;case"triangle-up":d3Symbol=d3.symbolTriangle;break;case"triangle-down":d3Symbol=d3.symbolTriangle;break;case"square":d3Symbol=d3.symbolSquare;break;case"star":d3Symbol=d3.symbolStar;break;case"wye":d3Symbol=d3.symbolWye;break;default:d3Symbol=d3.symbolCircle;break;}
parent.append("path").attr("transform",function(d){return"translate("+x+", "+y+")";}).attr("class","symbol").attr("d",d3.symbol().type(d3Symbol).size(50)).style("fill",color).style("stroke",color);}
