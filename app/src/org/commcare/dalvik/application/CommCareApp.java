...
public class CommCareApp {
    ...
    private void updateAppRecord() {
        // uniqueId, displayName, and versionNumber will be missing, so pull them from the app's
        // profile file
        record.setUniqueId(getUniqueId());
        record.setDisplayName(getDisplayName());
        record.setVersionNumber(getVersionNumber());
    ...
    public void writeInstalled() {
        ...
        record.setVersionNumber(getVersionNumber());
        record.setResourcesStatus(areResourcesValidated());
        record.setPreMultipleAppsProfile(checkFromOldProfile());
        try {
            CommCareApplication._().getGlobalStorage(ApplicationRecord.class).write(record);
    ...
    /**
     * @return whether this CommCareApp is from a profile file that was generated by a
    ...
    /**
     * @return the version number of the build this app was generated by
     */
    private int getVersionNumber() {
        //If this record has already been assigned the version number, pull it from there
        int version = record.getVersionNumber();
        if (version == -1) {
            //Otherwise, trying pulling it from the profile file
            version =  getCommCarePlatform().getCurrentProfile().getVersion();
        }
        return version;
    }
