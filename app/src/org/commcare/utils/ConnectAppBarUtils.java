...
public class ConnectAppBarUtils {
    ...
    /**
     * Sets the list icon properties including visibility and click listener.
     *
     * @param view          The root view where the list icon is located.
     * @param isVisible     If true, the list icon will be visible; otherwise, it will be hidden.
     * @param clickListener The click listener for the list icon (can be null).
     */
    public static void setListIconWithCallback(View view, boolean isVisible, View.OnClickListener clickListener) {
        ImageButton listIcon = view.findViewById(R.id.list_icon);

        if (listIcon != null) {
            listIcon.setVisibility(isVisible ? View.VISIBLE : View.GONE);

            if (clickListener != null) {
                listIcon.setOnClickListener(clickListener);
            }
        }
    }

    /**
     * Sets the cloud sync icon properties including visibility and click listener.
     *
     * @param view          The root view where the cloud sync icon is located.
     * @param isVisible     If true, the cloud sync icon will be visible; otherwise, it will be hidden.
     * @param clickListener The click listener for the cloud sync icon (can be null).
     */
    public static void setCloudSyncIconWithCallback(View view, boolean isVisible, View.OnClickListener clickListener) {
        ImageButton cloudSyncIcon = view.findViewById(R.id.cloud_sync);

        if (cloudSyncIcon != null) {
            cloudSyncIcon.setVisibility(isVisible ? View.VISIBLE : View.GONE);

            if (clickListener != null) {
                cloudSyncIcon.setOnClickListener(clickListener);
            }
        }
    }
