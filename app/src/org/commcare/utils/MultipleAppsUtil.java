...
public class MultipleAppsUtil {
    ...
    private static ArrayList<ApplicationRecord> getVisibleAppRecords() {
        ...
        for (ApplicationRecord r : CommCareApplication.getInstance().getInstalledAppRecords()) {
            if (r.isVisible()) {
                visible.add(r);
            }
        }
    ...
    public static ApplicationRecord[] appRecordArray() {
        ArrayList<ApplicationRecord> appList = CommCareApplication.getInstance().getInstalledAppRecords();
