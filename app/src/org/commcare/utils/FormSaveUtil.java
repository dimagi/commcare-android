...
public class FormSaveUtil {
    ...
    public static Hashtable<String, String> getNamespaceToFilePathMap(Context context) {
        ...
        for (String xmlns : CommCareApplication.getInstance().getCommCarePlatform().getInstalledForms()) {
            Cursor cur = null;
            try {
                Uri formContentUri = CommCareApplication.getInstance().getCommCarePlatform().getFormContentUri(xmlns);
                ...
                if (cur != null && cur.moveToFirst()) {
                    ...
                    throw new RuntimeException("No form registered for xmlns at content URI: " +
                            CommCareApplication.getInstance().getCommCarePlatform().getFormContentUri(xmlns));
                }
            } finally {
                if (cur != null) {
                    cur.close();
