...
public class EntityListAdapter implements ListAdapter {
    ...
    public void loadCalloutDataFromSession() {
        OrderedHashtable<String, String> externalData =
                (OrderedHashtable<String, String>)CommCareApplication.getInstance()
                        .getCurrentSession()
    ...
    public void saveCalloutDataToSession() {
        if (isFilteringByCalloutResult) {
            CommCareApplication.getInstance().getCurrentSession().addExtraToCurrentFrameStep(SessionInstanceBuilder.KEY_ENTITY_LIST_EXTRA_DATA, calloutResponseData);
        }
    }
