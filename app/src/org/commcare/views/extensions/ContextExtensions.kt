...
/**
 * Resolves a color role declared on the theme, so code can name a role the way a layout's `?attr/`
 * does. Handles a role assigned either a color resource or a literal color.
 *
 * Throws when the theme does not declare [attr]: an unresolved role would otherwise resolve to a
 * transparent color and go unnoticed until someone looked at the screen.
 */
