...
class LearningAppProgressResponseParser<T> : BaseApiResponseParser<T> {
    override fun parse(
        ...
        val job: ConnectJobRecord = anyInputObject as ConnectJobRecord
        try {
            responseData.use { `in` ->
                ...
                if (responseAsString.length > 0) {
                    ...
                    for (i in 0 until modules.length()) {
                        ...
                        val record = ConnectJobLearningRecord.fromJson(obj, job)
                    ...
                    for (i in 0 until assessments.length()) {
                        ...
                        val record = ConnectJobAssessmentRecord.fromJson(obj, job)
