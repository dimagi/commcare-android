...
public class LegacyInstallUtils {
    ...
    public static void checkForLegacyInstall(Context c, SqlStorage<ApplicationRecord> currentAppStorage) {
        ...
        if (installedApps == 0) {
            ...
            if (record == null) {
                globalPreferences.edit().putString(LEGACY_UPGRADE_PROGRESS, UPGRADE_COMPLETE).commit();
                Logger.log(LogTypes.TYPE_MAINTENANCE, "Legacy| app was detected, but new install already covers it");
                return;
            }
            ...
            globalPreferences.edit().putString(LEGACY_UPGRADE_PROGRESS, UPGRADE_COMPLETE).commit();
            Logger.log(LogTypes.TYPE_MAINTENANCE, "Legacy| More than one app record installed, skipping legacy app detection");
            return;
        }
        ...
        if (hasProfile && allInstalled) {
            ...
            globalPreferences.edit().putString(LEGACY_UPGRADE_PROGRESS, UPGRADE_COMPLETE).commit();
            Logger.log(LogTypes.TYPE_MAINTENANCE, "Legacy app detected, but it wasn't fully installed");
            return;
        }

        Logger.log(LogTypes.TYPE_MAINTENANCE, "Legacy| application installed. Beginning transition");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "Legacy| Global resources copied");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "Legacy| Prefs updated. Adding user records...");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "Legacy| " + oldUsers.size() + " old user records detected");
        ...
        for (User u : oldUsers) {
            ...
            if (newUserKeyRecords.getIDsForValue(UserKeyRecord.META_USERNAME, u.getUsername()).size() > 0) {
                Logger.log(LogTypes.TYPE_MAINTENANCE, "Legacy| Skipping key record for " + u.getUsername() + " . One already exists?");
                continue;
            }

            //See if we can find a user who was the last to log in
            if (preferred == null || u.getUsername().toLowerCase().equals(
                    globalPreferences.getString(HiddenCommCarePreferences.LAST_LOGGED_IN_USER, "").toLowerCase())) {
                preferred = u;
            }
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "Legacy| App succesfully Transitioned! Writing progress");
        ...
        app.getAppPreferences().edit().putLong(HiddenCommCarePreferences.LOG_LAST_DAILY_SUBMIT, 0).commit();
    ...
    public static void transitionLegacyUserStorage(final Context c, CommCareApp app, final byte[] oldKey, UserKeyRecord ukr) {
        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| Beginning transition attempt for " + ukr.getUsername());
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| Legacy DB Opened");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| Testing keys by attempting to open storage");
        ...
        try {
            ...
            e.printStackTrace();
            Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| Exception " + e.getMessage() + " when testing storage. Keys are probably no good");
            //This almost certainly means that we don't have the right key;
            return;
        }

        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| Old keys look good! Creating new DB");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| All set to get going. Beginning storage copy");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| " + legacyCases.getNumRecords() + " old cases detected");

        Map m = SqlStorage.cleanCopy(legacyCases,
                new SqlStorage<>(ACase.STORAGE_KEY, ACase.class, newAndroidDbHelper));

        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| " + m.size() + " cases copied. Copying Users");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| Sessions copied. Copying geocaches.");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| geocaches copied. Copying serialized log submissions.");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| serialized log submissions copied. Copying fixtures");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| Whew, storage copied! Updating key record");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| Eliminating shared data from old install, since new users can't access it");
        ...
        Logger.log(LogTypes.TYPE_MAINTENANCE, "LegacyUser| User transitioned! Closing db handles.");
