...
public abstract class CommCareActivity<R> extends FragmentActivity
    ...
    private void showPendingUserMessage() {
        String[] messageAndTitle = CommCareApplication.getInstance().getPendingUserMessage();
        if (messageAndTitle != null) {
            ...
            CommCareApplication.getInstance().clearPendingUserMessage();
        }
    }
    ...
    protected void restoreLastQueryString() {
        lastQueryString = (String)CommCareApplication.getInstance().getCurrentSession().getCurrentFrameStepExtra(SessionInstanceBuilder.KEY_LAST_QUERY_STRING);
    }

    protected void saveLastQueryString() {
        CommCareApplication.getInstance().getCurrentSession().addExtraToCurrentFrameStep(SessionInstanceBuilder.KEY_LAST_QUERY_STRING, lastQueryString);
    }
    ...
    protected static String getTitle(Context c, String local) {
        ...
        try {
            stepTitles = CommCareApplication.getInstance().getCurrentSession().getHeaderTitles();

            //See if we can insert any case hacks
            int i = 0;
            for (StackFrameStep step : CommCareApplication.getInstance().getCurrentSession().getFrame().getSteps()) {
                try {
                    if (SessionFrame.STATE_DATUM_VAL.equals(step.getType())) {
                        //Haaack
                        if (step.getId() != null && step.getId().contains("case_id")) {
                            ACase foundCase = CommCareApplication.getInstance().getUserStorage(ACase.STORAGE_KEY, ACase.class).getRecordForValue(ACase.INDEX_CASE_ID, step.getValue());
