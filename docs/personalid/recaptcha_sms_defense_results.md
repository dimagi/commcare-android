...
partial day. The metric tables were read at `2026-09-02T08:16Z` and the send verification code status at
`2026-09-02T09:49Z`, so counts are not directly comparable across those two sections.
...
curl -s -G \
  ...
  --data-urlencode 'interval.endTime=2026-09-02T08:16:00Z' \
...
| 2026-09-02 (partial) | 24 | 15 | 8 | 1 | 63% |
| **Total** | **203** | **145** | **57** | **1** | **71%** |
...
| `0.7` – `0.8` | 1 | — | — | 1 |
| **`≥ 0.8`** | **0** | **0** | **0** | **0** |
| Samples | 82 | 48 | 16 | 146 |

91% of scored traffic sits below `0.2`, and **nothing has reached `0.8` on any day**. Enforcing at
the current provisional `0.8` would therefore block none of these 146 requests — safe for users, and
...
`SendVerificationCode`, 234 calls in the window:

| Outcome | Count | Share |
| --- | --- | --- |
| `SUCCESS` | 189 | 81% |
| `INVALID_APP_CREDENTIAL` | 14 | 6% |
| `OPERATION_NOT_ALLOWED` — region not enabled | 14 | 6% |
| `Error code: 39` | 5 | 2% |
| `ALTERNATE_CLIENT_IDENTIFIER_REQUIRED` — invalid Play Integrity token | 5 | 2% |
| `MISSING_RECAPTCHA_TOKEN` | 4 | 2% |
| `TOO_MANY_ATTEMPTS_TRY_LATER` | 3 | 1% |
...
**Region rejections and app-credential failures are the joint-largest failure categories**, at 6%
each — roughly three times error 39. `smsRegionConfig` disallows only `CN`, so the region rejections
...
(`2026-08-27T11:55Z`, when SMS defense was enabled on the key) there have been **326 scored requests
and none at `≥ 0.8`** — the highest band reached is `0.7`–`0.8`, twice, leaving a full band of
headroom. Over the same period `passed` tracks `valid` exactly (325 against 326, the difference being
