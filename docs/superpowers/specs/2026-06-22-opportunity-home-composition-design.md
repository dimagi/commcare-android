...
This spec covers the activity's composition, lifecycle, the removal of two intermediate base classes (`SyncCapableCommCareActivity`, `HomeScreenBaseActivity`) that today bake the "session must exist" assumption into inheritance, and the state resolution / session-management policy that decides which status fragment to show and when to establish a session (see [State resolution and session management](#opportunity-home-state-resolution-and-session-management)). Entry points/routing, the inline silent-login mechanics themselves, and rollout remain out of scope and handled in parallel designs.
...
- **Sidesteps single inheritance entirely.** The `BaseDrawerActivity`-rooted new activity and the `SessionAwareCommCareActivity`-rooted existing home activities hold the *same* coordinator — one source of truth — without a shared superclass and without touching `SessionAwareCommCareActivity` (non-goal preserved).
...
**Host interface, not concrete activity.** The coordinator and its delegates need a handful of host capabilities — `Context`, `lifecycle`, `savedStateRegistry`, `startActivityForResult`, `showAlertDialog`, `rebuildOptionsMenu`, an optional UI-refresh hook (used by `changeLanguage()`; `OpportunityHomeActivity` has no `WithUIController`), and the `gating` abstraction described next. These are exposed through a small `HomeActivityHost` interface that `OpportunityHomeActivity`, `StandardHomeActivity`, and `RootMenuHomeActivity` all implement, keeping the coordinator unit-testable and free of any concrete-activity coupling.
...
### Removal of the intermediate base classes

Once the five behaviors live in the coordinator and its delegates, `SyncCapableCommCareActivity` and `HomeScreenBaseActivity` have almost nothing left to hold — what remains is thin host wiring (own a coordinator, implement `HomeActivityHost`, register it on the lifecycle, forward `onActivityResult`, call `attachSession(...)` once). Rather than keep two intermediate base classes as coordinator-holding shells, both are **removed**, and their two concrete subclasses — `StandardHomeActivity` and `RootMenuHomeActivity` — extend `SessionAwareCommCareActivity` directly and compose the coordinator themselves. This keeps a single source of truth (the coordinator, shared by all three home activities) without an inheritance layer whose only remaining job is to forward to it.

Concretely, each of the two existing home activities:

- extends `SessionAwareCommCareActivity` directly (keeping the session gate and its redirect-to-login-on-missing-session behavior, unchanged);
- holds one `HomeActivityCoordinator` and implements `HomeActivityHost`, registering the coordinator on its lifecycle so it observes directly, and forwarding the residual non-lifecycle callbacks (`onActivityResult`);
- calls `attachSession(...)` once in `onCreateSessionSafe` and never detaches — a gated activity cannot reach a session-less state; only `OpportunityHomeActivity` attaches and detaches.

`StandardHomeActivity` stays externally unchanged (and `WithUIController` remains `StandardHomeActivity`-only): its `onOptionsItemSelected` becomes a thin dispatch to coordinator actions (`coordinator.viewSavedForms()`, etc.) and its `onPrepareOptionsMenu` gating becomes per-action availability queries (`coordinator.canViewSavedForms()`, etc.), but the menu it presents is the same. `RootMenuHomeActivity`, the second (untested) host, absorbs the same wiring. The small amount of non-session host code the two classes previously shared through `HomeScreenBaseActivity` is thin enough to live directly in each concrete activity — which is why removal is preferred over retaining the shells.
...
- **Behavior drift between the chain and the new activity.** Mitigated by making all three home activities hold the same `HomeActivityCoordinator` (and therefore the same delegates and action bodies) — one source of truth. Confining `StandardHomeActivity` / `RootMenuHomeActivity` to thin forwarding into the coordinator minimizes the chance of semantic divergence.
...
- **Existing tests may assert against the removed classes' internal state or reference them as types.** Because `SyncCapableCommCareActivity` / `HomeScreenBaseActivity` no longer exist, any test that names them as a type (e.g. `FormAndDataSyncerFake`; see [Harness prerequisites](#harness-prerequisites)) must be migrated to `StandardHomeActivity` / `RootMenuHomeActivity`, the coordinator, or a delegate. Verify during implementation that behavioral tests against `StandardHomeActivity` still pass once it composes the coordinator directly.
...
This refactor moves roughly 2,300 lines of session-dependent behavior out of `HomeScreenBaseActivity` (1,828 lines) and `SyncCapableCommCareActivity` (495 lines) into a coordinator and five delegates, then removes both now-emptied classes and folds their residual host wiring into `StandardHomeActivity` and `RootMenuHomeActivity` (see [Removal of the intermediate base classes](#removal-of-the-intermediate-base-classes)). The §Risks section already flags the central hazard — behavior drift between the current chain and the migrated home activities. Today there is almost nothing to catch that drift. **Before any production code changes, a characterization ("pinning") test suite should be written against the current, observable behavior of the home activities**, so that the same tests, run unchanged against the migrated activities, prove the refactor is behavior-preserving.

### Guiding principle

Because the refactor keeps `StandardHomeActivity`'s and `RootMenuHomeActivity`'s **observable behavior unchanged** (see [Removal of the intermediate base classes](#removal-of-the-intermediate-base-classes)) — they compose the coordinator instead of inheriting the behavior, but present the same intents, dialogs, and menus — tests written against the **observable behavior** of `StandardHomeActivity` — next-started intents, dialogs shown, menu-item visibility, fragments swapped, saved/restored instance state — survive the refactor and become its contract. The corollary is a hard rule, and it is the same hazard §Risks names under "Existing tests may assert against the removed classes' internal state": **pin behavior, never internal fields or private methods.** Tests coupled to internal structure — or to the removed base-class types — will break on the migration and produce false regressions instead of catching real ones. Any such existing assertions should be identified and reworked into behavioral assertions first.
...
5. **Pin session expiration** = the current redirect-to-login behavior for `StandardHomeActivity`, so the migration demonstrably preserves it (and, by contrast, documents the behavior `OpportunityHomeActivity` deliberately diverges from).

Coverage should include `StandardHomeActivity` at minimum; note that `RootMenuHomeActivity` (today a second subclass of `HomeScreenBaseActivity`) has no coverage at all, so the refactor silently affects that path too.
...
- **Migrate `FormAndDataSyncerFake` off the removed type.** Its overrides take `SyncCapableCommCareActivity`, which this refactor removes, so the fake must be re-pointed at the new seam (the host setter / `SyncDelegate`) regardless; do it in the same commit as the seam.
