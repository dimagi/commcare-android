...
**Steps 8–9 (PIN and drift checks) are coordinator-owned, not delegates.** Unlike the other step bodies, steps 8 (`checkForPinLaunchConditions(session)`) and 9 (`checkForDrift()`) are private coordinator methods rather than delegate calls — there is no "PIN delegate" or "drift delegate." Both are one-shot, side-effect-only launch checks with no listener, no lifecycle callback, and no persistent instance state: the same category as the coordinator's other small dialog/launch bodies (`showAbout()`, `setPin()`, etc.). Forcing a `DefaultLifecycleObserver` delegate for either would add ceremony without benefit.

Step 8 reads launch-intent state the coordinator already owns (`LoginActivity.LOGIN_MODE`, `MANUAL_SWITCH_TO_PW_MODE`) plus the seated app's preferences and the current user record — hence it takes the `session` — then either launches the create-PIN screen or shows the PIN choice dialog. It is the automatic post-login counterpart to the user-invoked `setPin()` action, so both PIN entry points live together in the coordinator.
...
- **`DemoUserRestoreTest.java`** — its `checkOptionsMenuVisibility()` is the **only** assertion guarding demo-mode menu gating (change-language visible; update / saved-forms / preferences / advanced / about / set-pin hidden). This maps directly to the riskiest relocation in this spec (verify-item 5: per-item `!isDemoUser()` → coordinator availability queries), but it is buried in a heavyweight end-to-end test (`LEGACY` looper, an analytics-disabled workaround for an infinite loop) whose purpose is demo restore/update.
