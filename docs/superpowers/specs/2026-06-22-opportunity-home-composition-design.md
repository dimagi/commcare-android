...
| `AppUpdateDelegate` | `HomeScreenBaseActivity` (`AppUpdateController`) | App update prompts. **Session-independent** (see below): constructed once and registered on the host lifecycle like `CrashRecoveryDelegate`; only the surfacing of prompts is gated on a session existing. (Drift checks are *not* here — they carry no state or lifecycle and stay in the coordinator; see the drift note under [`HomeActivityCoordinator`](#homeactivitycoordinator).) |
