...
2. **App-wide back navigation** — one consistent, temporally-driven back model spanning the Intro page, the top-level navigation drawer, and the app-home surfaces. It is **flag-free in the north-star**; the pragmatic interim still relies on today's flags (`appLaunchedFromConnect`, `finishAffinity`, `REORDER_TO_FRONT`) until the shell lands (see Technical gap analysis).
...
| Installed apps present | [`MultipleAppsUtil.usableAppsPresent()`](https://github.com/dimagi/commcare-android/blob/dc7697645fefd99de4e234be569bd8447fb6e0ba/app/src/org/commcare/utils/MultipleAppsUtil.java#L50) |
| Last accessed opportunity | **new persistence — see below** |
| Last session context | **new persistence — see below** |

**Precedence (first match wins):**

1. **Explicit intent-driven startup — handled first.** External `ACTION_VIEW` app installs (via `CommCareSetupActivity`), verification refreshes carrying `KEY_REQUIRE_REFRESH` (via `CommCareVerificationActivity`), Connect deep links, and push-notification intents are resolved before any configuration-based routing — the resolver must not swallow them. These remain `DispatchActivity`'s responsibility in the interim (see Architecture).
2. **Active session alive → resume its home, no login shown.** Resume by the session's persisted login provenance (the **last session context** marker): a PersonalID-established session on an opportunity-linked app → `OpportunityHomeActivity` for that opportunity; a manual or non-opportunity session → `StandardHomeActivity` / `RootMenuHomeActivity`. **Tie-break:** if the persisted provenance disagrees with the seated app's current `evaluateAppState` linkage, provenance wins (linkage may be stale).
3. **No active session → resolve by configuration** (table below). Entering a Connect/PersonalID surface may require a **PersonalID unlock** (biometric/PIN via [`PersonalIdUnlocker`](https://github.com/dimagi/commcare-android/blob/dc7697645fefd99de4e234be569bd8447fb6e0ba/app/src/org/commcare/personalId/PersonalIdUnlocker.kt)). On a **warm** entry (a surface already visible beneath), cancel/failure dismisses the prompt and leaves that screen unchanged (both route through the same `connectActivityComplete(false)` path today); on a **cold start**, where nothing is beneath, cancel/failure **exits the app** rather than falling back to Intro or launching a partially-initialized home. Traditional CommCare users never hit this gate — they have no PersonalID. The router only *lands* on `OpportunityHomeActivity`; [#3776](https://github.com/dimagi/commcare-android/pull/3776)'s controller performs the in-place silent app-login on resume.
...
These are stored in a new dedicated shared-preferences store **scoped to the active PersonalID account**: entries are namespaced per account, cleared on `forgetUser()` and on account switch, and never inherited across accounts — so a newly-registered user cannot resume into a previous user's navigation state. Any pre-existing unscoped values are discarded on first run under the new store.
...
- **Session expiry** routes explicitly (never via Back) and must be **foreground-visible even when the app resumed from the background**: a **Standard Home** session that expires returns to the **CommCare Apps list** (not the removed Login page); an **Opp Home** session that expires detaches and **silently re-establishes on the next resume** per [#3776](https://github.com/dimagi/commcare-android/pull/3776) — it does *not* drop to Opp List. Neither relies on `DispatchActivity` re-dispatch.
- **Forget PersonalID** de-registers the account (re-registration required) and returns to Intro (via [`forgetUser()`](https://github.com/dimagi/commcare-android/blob/dc7697645fefd99de4e234be569bd8447fb6e0ba/app/src/org/commcare/connect/PersonalIdManager.java#L175) → `DispatchActivity` with `CLEAR_TASK`). Because forget is only reachable by a PersonalID user — whose active session is PersonalID-backed — de-registration must **also close that active app session** (`closeUserSession()`) before routing to Intro, so no PersonalID-less session is left dangling. (`forgetUser()` does not do this today; this spec adds it.)
...
- **Startup boundary regressions** — cover external `ACTION_VIEW` app install, verification success carrying `KEY_REQUIRE_REFRESH`, cold-start unlock cancellation (→ app exit), session expiry while backgrounded, Forget PersonalID with an active app session, and feature-flag-off behavior for traditional users.
