...
- **`PostLoginRouter`** — pure Kotlin, no Android dependencies.
  ...
  - `FailureReason` sealed class: `object BadCredentials`, `object LinkedAppRecordMissing`, `object TokenDenied`, `object AuthOverHttpBlocked`, `object AppSeatFailed`, `data class SyncFailed(val reason: String)`, `object NetworkUnavailable`, `object AlreadyLaunching`. (Parameterless reasons are `object`s; only `SyncFailed` carries data.)
...
- Routing per outcome:
  ...
  | `Failed(BadCredentials)`, `Failed(LinkedAppRecordMissing)`, `Failed(AuthOverHttpBlocked)` | Log via `Logger` + Firebase with a specific reason string (`"connect_login_bad_credentials"`, `"connect_login_linked_record_missing"`, `"connect_login_auth_over_http"`), then show the same retry/cancel dialog used for `SyncFailed` / `NetworkUnavailable`. These outcomes are not expected in normal use — `LinkedAppRecordMissing` shouldn't happen, and `AuthOverHttpBlocked` is a programming-safety guard that won't fire against a properly configured HQ. If a user genuinely gets stuck after retrying, the existing "Forget PersonalID" flow is the manual recovery surface                                                                                                                                                                                                                       |
...
- `BadCredentials` / `LinkedAppRecordMissing` / `AuthOverHttpBlocked`: each is logged via `Logger` + Firebase with the distinct reason string; the fragment shows the same retry/cancel dialog used for `SyncFailed` / `NetworkUnavailable`; no `forgetUser` call is made from the silent path.
