...
- **`LoginError`** — sealed class describing the failure mode. Splitting `Failed(LoginError)` out from the top-level result keeps callers' `when` blocks shallow when they only care about success vs failure, and lets new callers (Phase 3 fragment, Phase 4 fragments) opt into exhaustive error handling without each one re-listing every variant:
  - `BadCredentials` — both local and remote auth failed (includes `HttpCalloutOutcomes.IncorrectPin`, which fires only in PIN mode when no key record matches)
  ...
  - `NetworkUnavailable` — includes `HttpCalloutOutcomes.TokenUnavailable`.
  - `AuthOverHttpBlocked` — safety guard: auth callout attempted over plain HTTP ([`HttpCalloutTask.java:98-100`](https://github.com/dimagi/commcare-android/blob/684512e2662996c9855e35af306da3e311e56c80/app/src/org/commcare/network/HttpCalloutTask.java#L98)).
  ...
  **Mapping from existing task outcomes** (use this verbatim — implementers should not invent new mappings). Source enums: [`HttpCalloutOutcomes`](https://github.com/dimagi/commcare-android/blob/684512e2662996c9855e35af306da3e311e56c80/app/src/org/commcare/network/HttpCalloutTask.java#L37) (from [`ManageKeyRecordTask`](https://github.com/dimagi/commcare-android/blob/684512e2662996c9855e35af306da3e311e56c80/app/src/org/commcare/tasks/ManageKeyRecordTask.java)) and [`PullTaskResult`](https://github.com/dimagi/commcare-android/blob/684512e2662996c9855e35af306da3e311e56c80/app/src/org/commcare/tasks/DataPullTask.java#L716). Non-`Success` rows produce `Failed(<variant>)`; the `Failed(...)` wrapping is elided in the table for brevity.
  ...
  | `HttpCalloutOutcomes.AuthFailed`, `IncorrectPin`, `PullTaskResult.AUTH_FAILED` | `BadCredentials` |
  | `HttpCalloutOutcomes.NetworkFailure`, `NetworkFailureBadPassword`, `CaptivePortal`, `TokenUnavailable` | `NetworkUnavailable` |
  | `HttpCalloutOutcomes.TokenRequestDenied` | `TokenDenied` |
  | `HttpCalloutOutcomes.AuthOverHttp` | `AuthOverHttpBlocked` |
...
- **`ConnectCredentialResolver`** — ports [`ConnectAppUtils.getPasswordOverride()`](https://github.com/dimagi/commcare-android/blob/684512e2662996c9855e35af306da3e311e56c80/app/src/org/commcare/connect/ConnectAppUtils.kt#L69):
  ...
  Returns a non-null `ResolvedCredentials` on success. If a record cannot be located or created (e.g., DB write failure), throws `IllegalStateException`. This matches today's `ConnectAppUtils.getPasswordOverride()`, which throws `RuntimeException` in the equivalent unreachable case — `ConnectAppLauncher` lets the exception propagate as a programming-error crash. Existing `ConnectAppUtils` call sites delegate.
...
- **`PostLoginRouter`** — pure Kotlin, no Android dependencies.
  ...
  - `FailureReason` sealed class: `object BadCredentials`, `object TokenDenied`, `object AuthOverHttpBlocked`, `object AppSeatFailed`, `data class SyncFailed(val reason: String)`, `object NetworkUnavailable`, `object AlreadyLaunching`. (Parameterless reasons are `object`s; only `SyncFailed` carries data.)
...
- Routing per outcome:
  ...
  | `Failed(BadCredentials)`, `Failed(AuthOverHttpBlocked)` | Log via `Logger` + Firebase with a specific reason string (`"connect_login_bad_credentials"`, `"connect_login_auth_over_http"`), then show the same retry/cancel dialog used for `SyncFailed` / `NetworkUnavailable`. `AuthOverHttpBlocked` is a programming-safety guard that won't fire against a properly configured HQ. If a user genuinely gets stuck after retrying, the existing "Forget PersonalID" flow is the manual recovery surface                                                                                                                                                                                                                       |
...
- `BadCredentials` / `AuthOverHttpBlocked`: each is logged via `Logger` + Firebase with the distinct reason string; the fragment shows the same retry/cancel dialog used for `SyncFailed` / `NetworkUnavailable`; no `forgetUser` call is made from the silent path.
