...
`AppSeater.seatIfNeeded(appId, sink)` seats an app into `CommCareApplication` off the UI thread, emitting the `Seating` phase, and returns a `SeatResult` (`Success`, or `Failed(SeatFailure.APP_NOT_FOUND | CORRUPTED)`). `SeatAppActivity` and the Connect launch path both seat through it rather than on the activity stack; Java callers use `start(lifecycleOwner, appId, sink, callback)`.
