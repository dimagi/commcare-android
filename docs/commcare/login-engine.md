...
`ConnectAppLaunchUiController` is the fragment-side orchestration shared by every Connect launch surface: it shows a modal `CustomProgressDialog` (mirroring `LoginActivity`'s spinner-then-bar), runs `ConnectAppLauncher` on the fragment's `viewLifecycleOwner` scope, and dispatches the outcome through `LaunchOutcomeRouter`. On success it starts Home (via `HomeScreenBaseActivity.buildHomeLaunchIntent`) on top of the Connect screen rather than through `DispatchActivity`, so backing out of Home returns to the originating Connect screen. Home is launched *for result* so that the in-app "View Job Status" button — which finishes Home with `REDIRECT_TO_CONNECT_OPPORTUNITY_INFO` — routes to the seated app's job status page via `ConnectNavHelper.goToActiveInfoForJob`, matching `DispatchActivity`'s handling of that result on the legacy path. `ConnectJobsListsFragment` (opportunities list), `ConnectDeliveryProgressFragment` (delivery app), and `ConnectLearningProgressFragment` (learn app) all launch through it. The controller is built as a fragment field (registering its result launcher at construction) and observes the fragment's view lifecycle to dismiss the dialog when the view is destroyed, so callers don't manage cleanup.
