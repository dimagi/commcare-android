...
`ConnectAppLaunchController` is the fragment-side orchestration shared by every Connect launch surface: it shows a modal `CustomProgressDialog` (mirroring `LoginActivity`'s spinner-then-bar, with copy from the `connect_app_launch_dialog_*` `strings.xml` resources — a constant title and a phase-specific message), runs `ConnectAppLauncher` on the fragment's `viewLifecycleOwner` scope, and dispatches the outcome through `LaunchOutcomeRouter`. On success it starts Home (via `HomeScreenBaseActivity.buildHomeLaunchIntent`, with `FLAG_ACTIVITY_CLEAR_TOP`) on top of the Connect screen rather than through `DispatchActivity`. Home is started normally (not for result): both a back-out and an in-app logout let the activity back stack return the worker to the screen they launched from. The app session is left alive on a back-out — it ends only on the launched app's own logout (`HomeScreenBaseActivity.userTriggeredLogout`) or when the session window expires. Every Connect launch surface launches through it: `ConnectJobsListsFragment` (opportunities list), `ConnectDeliveryProgressFragment` (delivery app), `ConnectLearningProgressFragment` (learn app), `ConnectJobIntroFragment` (start-learning launch), and `ConnectDownloadingFragment` (post-install launch). The controller does not manipulate the navigation back stack itself; instead it invokes an optional `onLaunchSucceeded` callback once Home has started, letting each caller manage its own lifecycle. The two transient setup surfaces (`ConnectJobIntroFragment` and `ConnectDownloadingFragment`) pass `BaseConnectFragment.popSelfOnceHidden()`, which pops them off the Connect back stack once Home covers them, so backing out of the launched app returns to the screen beneath them rather than the setup screen. The controller observes the fragment's view lifecycle to dismiss the dialog when the view is destroyed, so callers don't manage cleanup.

### Anchoring back to the user's base screen

`ConnectAppLaunchController` sets the session-scoped `CommCareApplication.sessionLaunchedFromConnect` flag when it starts Home; `closeUserSession()` clears it. `DispatchActivity` routes an active session carrying this flag to `LoginActivity` rather than Home, so backing out of (or later reopening) a Connect-launched app returns the user to their base screen with the session intact.

### Back navigation between Home and the Connect job-status pages
