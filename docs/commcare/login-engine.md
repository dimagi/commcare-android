...
The two remaining `ConnectAppUtils.launchApp` callers (`ConnectJobIntroFragment`, `ConnectDownloadingFragment`) still take the legacy `LoginActivity` path; `ConnectAppUtils.launchApp` itself is unchanged and also serves as the Connect launch path's fallback for credential and retryable failures.
